#!/bin/bash
# count_chip_peaks.sh
# Count ChIP-seq fragments over paired P1/P2 peaks on the concatenated
# reference.
#
# Required inputs:
#   BAM          coordinate-sorted, deduplicated ChIP BAM
#   PEAKS        BED with prefixed P1/P2 peak coordinates and names in col 4
#   PEAK_PAIRS   TSV pairing P1 and P2 peak names
#
# Outputs (OUTDIR, default results/counts/chip/${LIBRARY_ID}):
#   peak_counts.tsv
#   filter_summary.tsv
#
...
BAM=${BAM:-results/mapping/chip/${LIBRARY_ID}.dedup.unique.bam}
PEAKS=${PEAKS:-chip_peaks.bed}
PEAK_PAIRS=${PEAK_PAIRS:-${REGION_PAIRS:-chip_region_pairs.tsv}}
OUTDIR=${OUTDIR:-results/counts/chip/${LIBRARY_ID}}
COUNT_TMPDIR=${COUNT_TMPDIR:-${CHIP_TMPDIR:-chip_count_tmp/${LIBRARY_ID}}}
...
[ -n "$SAMPLE" ] || die "SAMPLE must not be empty"

log "ChIP peak counting: ${LIBRARY_ID}"
echo "  BAM        = $BAM"
echo "  PEAKS      = $PEAKS"
echo "  PEAK_PAIRS = $PEAK_PAIRS"
echo "  OUTDIR     = $OUTDIR"

for f in "$BAM" "$PEAKS" "$PEAK_PAIRS"; do
...
log "Filtering BAM to unique, properly paired, primary, non-duplicate alignments"
TOTAL_BAM_RECORDS=$("$SAMTOOLS" view -c "$BAM")
"$SAMTOOLS" view -b -q "$MAPQ_MIN" -f 0x2 -F 0xD04 "$BAM" \
...
awk -v prefix="$P1_PREFIX" 'BEGIN { FS=OFS="\t" } $1 ~ "^" prefix' "$PEAKS" \
    | sort -k1,1 -k2,2n > "$COUNT_TMPDIR/P1_peaks.bed"
awk -v prefix="$P2_PREFIX" 'BEGIN { FS=OFS="\t" } $1 ~ "^" prefix' "$PEAKS" \
    | sort -k1,1 -k2,2n > "$COUNT_TMPDIR/P2_peaks.bed"

for f in "$COUNT_TMPDIR/P1_fragments.bed" "$COUNT_TMPDIR/P2_fragments.bed" \
         "$COUNT_TMPDIR/P1_peaks.bed" "$COUNT_TMPDIR/P2_peaks.bed"; do
    [ -s "$f" ] || die "expected non-empty split file: $f"
done

log "Counting fragment overlaps per ChIP peak"
"$BEDTOOLS" intersect "${INTERSECT_F[@]}" \
    -a "$COUNT_TMPDIR/P1_peaks.bed" -b "$COUNT_TMPDIR/P1_fragments.bed" -c \
    > "$COUNT_TMPDIR/P1_peak_counts.bed"
"$BEDTOOLS" intersect "${INTERSECT_F[@]}" \
    -a "$COUNT_TMPDIR/P2_peaks.bed" -b "$COUNT_TMPDIR/P2_fragments.bed" -c \
    > "$COUNT_TMPDIR/P2_peak_counts.bed"

log "Pairing P1/P2 peak counts"
python3 - "$COUNT_TMPDIR/P1_peak_counts.bed" "$COUNT_TMPDIR/P2_peak_counts.bed" \
          "$PEAK_PAIRS" "$OUTDIR/peak_counts.tsv" <<'PYEOF'
import csv
import sys
...
def read_count_bed(path):
    ...
    with open(path) as source:
        for line_number, line in enumerate(source, start=1):
            ...
            peak_name = fields[3]
            if peak_name in counts:
                raise SystemExit(f"ERROR: duplicate peak name in {path}: {peak_name}")
            try:
                counts[peak_name] = int(fields[-1])
            except ValueError as error:
                raise SystemExit(
...
with open(pairs_path, newline="") as source:
    ...
    id_col = choose_column(reader.fieldnames, ("peak_pair_id", "region_id", "pair_id", "feature_id"), pairs_path)
    p1_col = choose_column(reader.fieldnames, ("P1_peak", "P1_region", "P1_feature"), pairs_path)
    p2_col = choose_column(reader.fieldnames, ("P2_peak", "P2_region", "P2_feature"), pairs_path)
    rows = list(reader)

seen_ids = set()
with open(output_path, "w", newline="") as output:
    writer = csv.writer(output, delimiter="\t", lineterminator="\n")
    writer.writerow(("peak_pair_id", "P1_peak", "P2_peak", "P1_count", "P2_count", "total_count"))
    for row in rows:
        pair_id = row[id_col]
        p1_peak = row[p1_col]
        p2_peak = row[p2_col]
        if not pair_id or pair_id in seen_ids:
            raise SystemExit(f"ERROR: blank or duplicate peak pair ID in {pairs_path}: {pair_id!r}")
        if p1_peak not in p1_counts:
            raise SystemExit(f"ERROR: P1 peak absent from P1 BED counts: {p1_peak}")
        if p2_peak not in p2_counts:
            raise SystemExit(f"ERROR: P2 peak absent from P2 BED counts: {p2_peak}")
        seen_ids.add(pair_id)
        p1_count = p1_counts[p1_peak]
        p2_count = p2_counts[p2_peak]
        writer.writerow((pair_id, p1_peak, p2_peak, p1_count, p2_count, p1_count + p2_count))
PYEOF
...
echo ""
log "ChIP peak counting complete: ${OUTDIR}"
echo ""
echo "--- filter_summary.tsv ---"
sed 's/^/  /' "$OUTDIR/filter_summary.tsv"
echo ""
echo "--- peak_counts.tsv ---"
show "$OUTDIR/peak_counts.tsv" | sed 's/^/  /'
