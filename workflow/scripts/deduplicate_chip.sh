#!/bin/bash
# deduplicate_chip.sh
# Remove paired-end PCR duplicates from one coordinate-sorted ChIP unique BAM.
#
...
WORKDIR=$(mktemp -d "${TMP_ROOT%/}/hyarq_chip_dedup.XXXXXX")
...
log "ChIP duplicate removal: ${LIBRARY_ID}"
echo "  input  = $BAM"
...
log "ChIP duplicate removal complete: ${LIBRARY_ID}"
